...
Output ONLY the email body (no subject line)."""
    ...
    requests.post(
        ...
        headers={"Authorization": f"Zoho-oauthtoken {zoho.token}",
                 "Content-Type": "application/json"},
        json={"text": (
            f"PERSONALIZATION RUN — Segment: {segment}\n"
            f"Goal: {campaign_goal}\n"
            f"Contacts: {len(contacts)}  |  "
            f"{'Sent' if send else 'Drafted'}: {sent or drafted}  |  Skipped (no email): {skipped}\n"
            f"Each email is unique — references individual history."
        )},
    )
